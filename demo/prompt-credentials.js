...
module.exports = function login(sessionPath = '/tmp/icloud-session.json') {
  return new Promise(function (resolve, reject) {
    ...
    myCloud.on('err', function (err) {
      if (err.errorCode == 6) {
        console.error(
          'Given session does not eixst or is expired. Try to use contained credentials from session to re-login...'
        );
